...
class EtapaControllerSpec extends Specification implements ControllerUnitTest<EtapaController>, DomainUnitTest<Etapa> {
    ...
    void "Test the index action returns the correct model"() {
        given:
        controller.etapaService = Mock(EtapaService) {
            1 * list(_) >> []
            1 * count() >> 0
        }
    ...
    void "Test the save action with an invalid instance"() {
        given:
        controller.etapaService = Mock(EtapaService) {
            1 * save(_ as Etapa) >> { Etapa etapa ->
                throw new ValidationException("Invalid instance", etapa.errors)
            }
        }
